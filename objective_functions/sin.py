...
class LinearSin(ObjectiveFunction):
    def __init__(self,
                 additional_gaussian_noise_std: float = 0.,
                 ):
        super(LinearSin, self).__init__(additional_gaussian_noise_std)
    ...
    def evaluate_without_noise(self,
        ...
        result = 0.5 * np.sin(x * 2 * np.pi / 5) + 1 - 0.3 * x
        return result
